...
const friendEventsOpen = ref(false);
const friendEventsList = ref([]);
const selectedFriend = ref(null);
const friendEventsPage = ref(1)
const friendEventsPerPage = 4

const totalFriendEventPages = computed(() =>
  Math.ceil(friendEventsList.value.length / friendEventsPerPage)
)

const paginatedFriendEvents = computed(() => {
  const start = (friendEventsPage.value - 1) * friendEventsPerPage
  return friendEventsList.value.slice(start, start + friendEventsPerPage)
})
...
const totalEventPages = computed(() =>
  Math.ceil(joinedEvents.value.length / eventPerPage)
);
...
async function openFriendEvents(friend) {
  selectedFriend.value = friend;
  friendEventsOpen.value = true;
  friendEventsList.value = [];
  friendEventsPage.value = 1
  ...
  try {
    ...
    friendEventsList.value = events || [];
  } catch (err) {
    console.error("openFriendEvents error:", err);
    toast.error("Failed to load friend's events.");
    friendEventsList.value = [];
  }
}
...
async function sendRequest(targetId) {
  ...
  const targetPending = Array.from(
    new Set([...(target?.friend_requests || []), user.value.id])
  );
...
async function acceptRequest(requesterId) {
  ...
  const requesterFriends = Array.from(
    new Set([...(requester?.friends || []), user.value.id])
  );
  const updatedMyRequests = (me?.friend_requests || []).filter(
    (id) => id !== requesterId
  );
  const updatedRequesterSent = (requester?.sent_requests || []).filter(
    (id) => id !== user.value.id
  );
...
async function declineRequest(requesterId) {
  ...
  const updatedMe = (me?.friend_requests || []).filter(
    (id) => id !== requesterId
  );
  const updatedRequester = (requester?.sent_requests || []).filter(
    (id) => id !== user.value.id
  );

  await supabase
    .from("user_preferences")
    .update({ friend_requests: updatedMe })
    .eq("id", user.value.id);
  await supabase
    .from("user_preferences")
    .update({ sent_requests: updatedRequester })
    .eq("id", requesterId);
...
async function removeFriendConfirmed() {
  try {
    ...
    const myFriends = (me?.friends || [])
      .map(String)
      .filter((id) => id !== String(friendId));
    const friendFriends = (friend?.friends || [])
      .map(String)
      .filter((id) => id !== String(user.value.id));
...
async function sendEventInvite(targetId, eventId) {
  try {
    ...
    const newSent = [
      ...(me?.sent_invites || []),
      { event_id: eventId, to: targetId },
    ];
    const newReceived = [
      ...(targetInvites?.invite_requests || []),
      { event_id: eventId, from: user.value.id },
    ];
...
async function acceptInvite(inv) {
  try {
    ...
    const updatedGoing = Array.from(
      new Set([...(me?.going || []), inv.event_id])
    );
    ...
    const myUpdated = (myInvites?.invite_requests || []).filter(
      (i) => !(i.event_id === inv.event_id && i.from === inv.from)
    );
    const senderUpdated = (senderInvites?.sent_invites || []).filter(
      (i) => !(i.event_id === inv.event_id && i.to === user.value.id)
    );

    const [{ error: e1 }, { error: e2 }] = await Promise.all([
      supabase
        .from("user_preferences")
        .update({ invite_requests: myUpdated })
        .eq("id", user.value.id),
      supabase
        .from("user_preferences")
        .update({ sent_invites: senderUpdated })
        .eq("id", inv.from),
    ]);
...
async function declineInvite(inv) {
  try {
    ...
    const myUpdated = (me?.invite_requests || []).filter(
      (i) => !(i.event_id === inv.event_id && i.from === inv.from)
    );
    const senderUpdated = (sender?.sent_invites || []).filter(
      (i) => !(i.event_id === inv.event_id && i.to === user.value.id)
    );
  ...
  <section class="min-h-screen py-12 px-6 md:px-12 xl:px-20 bg-white relative">
    ...
    <div class="max-w-md mx-auto mb-8 relative">
      <div class="flex gap-2 relative">
        ...
        <Button
          ...
          @click="
            searchQuery ? null : toast.info('Type a name or email to search!')
          "
        >
          Search
        </Button>
      ...
      >
        ...
        >
          ...
          <Button size="sm" class="cursor-pointer" @click="sendRequest(user.id)"
            >Request</Button
          >
        </li>
    ...
    <div class="max-w-4xl mx-auto mt-16">
      ...
      <Table>
        <TableCaption v-if="friends.length === 0 && !pageLoading"
          >You have no friends yet 😢</TableCaption
        >
    ...
    <Dialog v-model:open="requestsOpen">
      <DialogContent class="max-w-md">
        <DialogHeader
          ><DialogTitle>Pending Friend Requests</DialogTitle></DialogHeader
        >
        ...
        <div v-if="joinedEvents.length" class="space-y-3 mt-2">
          ...
          >
            ...
            <Button
              size="sm"
              class="cursor-pointer"
              @click="sendEventInvite(inviteTarget.id, ev.id)"
            >
              Invite
            </Button>
          </div>
    ...
    <!-- Friend Events Dialog -->
    <Dialog v-model:open="friendEventsOpen">
      <DialogContent class="max-w-lg">
        <DialogHeader>
          <DialogTitle>Events Joined by {{ selectedFriend?.name }}</DialogTitle>
        </DialogHeader>

        <div v-if="friendEventsList.length" class="divide-y divide-gray-200 mt-2">
          <!-- Paginated Events -->
          <div
            v-for="ev in paginatedFriendEvents"
            :key="ev.id"
