...
import { ArrowLeftIcon, ArrowPathIcon, ArrowDownTrayIcon, EyeIcon, ShareIcon } from '@heroicons/vue/24/outline'
...
function generateSchedulePDF() {
    ...
    parsedSchedule.value.forEach((item) => {
        console.log(parsedSchedule.value)
...
function matchEventsToSchedule(schedule, eventList) {
  const matchedEvents = []
  
  schedule.forEach((item, index) => {
    const activity = item.activity.toLowerCase()
    
    // Only match if the event title explicitly appears in the activity
    const match = eventList.find(event => {
      const eventTitle = event.title.toLowerCase()
      
      // Check if the full event title (or most of it) appears in activity
      return activity.includes(eventTitle)
    })
...
onMounted(async () => {
    ...
    const eventListSG = await getCat()
    ...
    matchedEvents.value = matchEventsToSchedule(parsedSchedule.value, eventListSG)

    console.log('Parsed schedule:', parsedSchedule.value)
    console.log('Event list:', eventListSG)
    console.log('Matched events:', matchedEvents.value)
})
</script>
            ...
            <div class="row">
                <div v-if="parsedSchedule.length" class="w-full">
                    ...
                    <ul class="space-y-4 mt-4">
                        <li v-for="(item, index) in parsedSchedule" :key="item.start"
                            ...
                            <!-- Show matched event link if available -->
                            <div v-if="matchedEvents.find(m => m.scheduleIndex === index)" class="mt-2">
                                <button
                                    @click="goToEventDetails(matchedEvents.find(m => m.scheduleIndex === index).matchedEvent.id)"
                                    class="text-sm text-blue-600 hover:text-blue-800 underline flex items-center gap-1">
                                    <EyeIcon class="h-4 w-4" />
                                    View Event Details: {{matchedEvents.find(m => m.scheduleIndex ===
                                        index).matchedEvent.title}}
                                </button>
                            </div>
                        </li>
                    </ul>
                ...
                <div v-else class="flex items-center justify-center min-h-[70vh]">
                    <div class="text-center">
                        <div class="w-30 h-30 rounded-full flex items-center justify-center mb-4 mx-auto">
                        ...
                        <div class="flex gap-3">
                            ...
                            <Dialog v-model:open="showNameModal">
                                ...
                                <DialogContent class="sm:max-w-md">
                                    ...
                                    <Input v-model="planName" placeholder="e.g. Weekend Brunch Plan" class="mt-3" />
